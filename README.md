...
- **Light Guide keyboard map**: While in Learning Mode, the LED strip maintains a persistent. Upcoming targets are clearly differentiated, pulsing pure white for white keys and warm-orange for black keys.
- **Demo Mode & MIDI Import**: Drop any standard `.mid` file into the app and it instantly digests it into playable, native learning steps without a server. Hit Play and the system acts as an autopilot, lighting up the physical LED strip and playing audio at original precision tempo.
